...
class Gene(GFFFeature):
    ...
    def from_gff_feature(feature):
        if feature.feature_type == "gene":
            return Gene(feature.seqid, feature.source, feature.start, feature.end, feature.score, feature.strand,
                        feature.phase, feature.attributes, feature.children)
        return None
    ...
    def fix_phase(self, bases):
        ...
        if "start_codon" not in self.get_mrna():
            if gene_start == 2:
                self.start = 1
        ...
        if "stop_codon" not in self.get_mrna():
            self.end = len(bases)
